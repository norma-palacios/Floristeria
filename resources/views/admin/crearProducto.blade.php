...
<div class="admin-content">
    ...
    <div class="d-flex align-items-center mb-4" style="background: #5A1E8F; padding: 15px 20px; border-radius: 12px; color: white;">
        <a href="{{ route('admin.productos') }}" style="color: white; font-size: 24px; margin-right: 15px; text-decoration: none;">
            ←
        </a>
        <h2 class="fw-bold" style="margin: 0;">Añadir producto</h2>
    </div>
        ...
        <div class="row">
            ...
            <div class="col-md-4 text-center mb-4">
                <div style="background: #E8E8E8; border-radius: 12px; padding: 40px; min-height: 350px; display: flex; flex-direction: column; align-items: center; justify-content: center; position: relative;">
                    <img id="previewImagen" src="https://via.placeholder.com/200x250" alt="Preview" style="max-width: 100%; max-height: 250px; border-radius: 8px; margin-bottom: 20px; object-fit: cover;">
                    ...
                    <div class="d-flex gap-2 justify-content-center">
                        <label class="btn fw-bold" style="background: #5A1E8F; color: white; cursor: pointer; margin-bottom: 0; border-radius: 6px; padding: 8px 16px;">
                            Subir
                            <input type="file" id="imagenInput" style="display: none;" accept="image/*">
                        </label>
                        
                        <button type="button" id="btnEliminarImagen" class="btn" style="background: #FFE0E0; color: #C91C1C; border: none; border-radius: 6px; padding: 8px 16px; font-weight: bold;" onclick="limpiarImagen()" style="display: none;">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="display: inline; margin-right: 5px;">
                                <path d="M19 6.41L17.59 5L12 10.59L6.41 5L5 6.41L10.59 12L5 17.59L6.41 19L12 13.41L17.59 19L19 17.59L13.41 12L19 6.41Z" fill="#C91C1C"/>
                            </svg>
                        </button>
                    </div>
                </div>
            ...
            <div class="col-md-8">
                <form action="{{ route('admin.productos.guardar') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nombre" class="form-control @error('nombre') is-invalid @enderror" placeholder="Ramo Adali" value="{{ old('nombre') }}" required style="border: 1px solid #DDD; border-radius: 6px; padding: 10px; font-size: 14px;">
                        @error('nombre')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="descripcion" class="form-control @error('descripcion') is-invalid @enderror" rows="4" placeholder="Hermoso Ramo floral de 5 gerberas, acompañadas de rosas, tulipanes y follaje adornado con papel coreano rose pastel y listón rosa" style="border: 1px solid #DDD; border-radius: 6px; padding: 10px; font-size: 14px; resize: vertical;" required>{{ old('descripcion') }}</textarea>
                        @error('descripcion')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" name="precio" class="form-control @error('precio') is-invalid @enderror" placeholder="0.00" step="0.01" min="0" value="{{ old('precio') }}" required style="border: 1px solid #DDD; border-radius: 0 6px 6px 0; padding: 10px; font-size: 14px;">
                            @error('precio')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="categoria" class="form-control @error('categoria') is-invalid @enderror" placeholder="cumpleaños, boda, san valentín, aniversario, funeral, sin ocasión especial" value="{{ old('categoria') }}" style="border: 1px solid #DDD; border-radius: 6px; padding: 10px; font-size: 14px;">
                        @error('categoria')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
...
<script>
    ...
    document.getElementById('imagenInput').addEventListener('change', function(e) {
        ...
        if (file) {
            ...
            reader.onload = function(event) {
                ...
                document.getElementById('btnEliminarImagen').style.display = 'inline-block';
            };
    ...
    function limpiarImagen() {
        ...
        document.getElementById('previewImagen').src = 'https://via.placeholder.com/200x250';
